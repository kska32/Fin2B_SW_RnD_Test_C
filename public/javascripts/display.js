...
console.log("%cThe Library Created By https://absolute.tk In April.2018 ",
...
let put_PosTables = {
    center:{pos:'center',x:'50%',y:'50%',tx:'-50%',ty:'-50%'},
    leftIn:{pos:'leftIn',x:'0%',y:'50%',tx:'0%',ty:'-50%'},
    rightIn:{pos:'rightIn',x:'100%',y:'50%',tx:'-100%',ty:'-50%'},
    topIn:{pos:'topIn',x:'50%',y:'0%',tx:'-50%',ty:'0%'},
    bottomIn:{pos:'bottomIn',x:'50%',y:'100%',tx:'-50%',ty:'-100%'},
    leftTopIn:{pos:'leftTopIn',x:'0%',y:'0%',tx:'0%',ty:'0%'},
    rightTopIn:{pos:'rightTopIn',x:'100%',y:'0%',tx:'-100%',ty:'0%'},
    leftBottomIn :{pos:'leftBottomIn',x:'0%',y:'100%',tx:'0%',ty:'-100%'},
    rightBottomIn: {pos:'rightBottomIn',x:'100%',y:'100%',tx:'-100%',ty:'-100%'},
    leftOut: {pos:'leftOut',x:'0%',y:'50%',tx:'-100%',ty:'-50%'},
    rightOut: {pos:'rightOut',x:'100%',y:'50%',tx:'0%',ty:'-50%'},
    topOut: {pos:'topOut',x:'50%',y:'0%',tx:'-50%',ty:'-100%'},
    bottomOut: {pos:'bottomOut',x:'50%',y:'100%',tx:'-50%',ty:'0%'},
    leftTopOut: {pos:'leftTopOut',x:'0%',y:'0%',tx:'-100%',ty:'-100%'},
    rightTopOut: {pos:'rightTopOut',x:'100%',y:'0%',tx:'0%',ty:'-100%'},
    leftBottomOut: {pos:'leftBottomOut',x:'0%',y:'100%',tx:'-100%',ty:'0%'},
    rightBottomOut: {pos:'rightBottomOut',x:'100%',y:'100%',tx:'0%',ty:'0%'}
}

 class SuperElement{//div
    constructor(el){
    ...
    keyframes(str,waitMs=0,aniName=undefined) {//wait for ms and then delete automatically.
        if(aniName === undefined) var aniName = this.__uniqueCode("kf");
    ...
    put(pos, cssPos = "absolute"){//pos = center|leftIn|rightIn|topIn|bottomIn 
        let el = this.element;
        if( this.getClassName(el).indexOf(pos) === -1 ){
            //if(el.className.indexOf(pos) === -1){
            let lst = put_PosTables[pos];
            if(internalCnList[lst.pos] === undefined){
                let cssText = `.${lst.pos}{
                    position:${cssPos};
                    left:${lst.x};
                    top:${lst.y};
                    transform: translate(${lst.tx},${lst.ty});
                }`;
                let cssTextNode = document.createTextNode(cssText);
                internalCssElement.appendChild(cssTextNode);
                internalCnList[lst.pos]=[];
            }
            internalCnList[lst.pos].push(el);
            this.putTXY = ` translate(${lst.tx}, ${lst.ty}) `;
            //el.className += `${lst.pos} `;
            this.addClassName(`${lst.pos}`,el);
        }
    }
    ...
    centerText(text=""){
        this.text(text);
        this.dressup({'text-align':'center','line-height':this.heightC});
    }
 ...
 class SvgElement extends SuperElement{
    constructor(eln){
        super();
